...
2. **Designer** (`/design`): a single product-page layout, the garment mockup sits sticky on
...
doesn't, the "livery" is deterministic layout and typography (`src/lib/livery.ts`): checkered
corners, carbon plate, neon lines, flame glow, or retro stripes as background, sponsors and a
name tag set in real display fonts on top. It's cheaper to run (no per-generation API cost),
